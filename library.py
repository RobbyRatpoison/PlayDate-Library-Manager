...
def game_description(appid):
    ...
    # A recent lookup already found nothing (delisted app, plugin with no blurb):
    # don't ask the store again until the retry window passes.
    if not description_lookup_due(row['short_description_checked']):
        return jsonify({'status': 'error', 'message': 'No description available'})
    platform = row['platform'] or 'steam'
    desc = ''
    answered = False   # True only when the store replied; errors/429s leave it False so we retry
    try:
        ...
        if answered:
            _ugd(appid, short_description_checked=datetime.now().strftime('%Y-%m-%d'))
    except Exception as e:
        return api_error('Something went wrong on the server. Check playdate.log for details.', 500, exc=e)
    return jsonify({'status': 'error', 'message': 'No description available'})
